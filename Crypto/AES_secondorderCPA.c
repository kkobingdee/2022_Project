#include <stdio.h>
#include <stdlib.h>
#include <string.h> 
#include <math.h>

#define _FOLD_ "C:\\Users\\부채널분석1\\Desktop\\Masking Trace\\"   
#define TraceFN "2022.02.07-06.49.10_traces.npy"
#define PlaintextFN "2022.02.07-06.49.10_textin.npy"

#define startpoint 0
#define endpoint 24000

static unsigned char Sbox[256] = {
   
};


void secondorderCPA() {
	char buf[256];
	FILE* rfp, * wfp;
	int err, TraceNum = 3000, TraceLength = 24000, i, j, k, key, maxkey;
	double** data, **Mult, **Min;
	double* Mean, * Sum, * Sx, * Sxx, * Sxy, * corrT;
	double Sy, Syy, max;
	unsigned char** plaintext = NULL;
	unsigned char x, y, iv, hw_iv;
	int S_Msbox = 5200;
	int E_Msbox = 5300;
	int S_Msub = 19700;
	int E_Msub = 19900;
	int len = (E_Msbox - S_Msbox) * (E_Msub - S_Msub);

	//data에 파형 저장
	sprintf_s(buf, 256 * sizeof(char), "%s%s", _FOLD_, TraceFN);
	if ((err = fopen_s(&rfp, buf, "rb"))) {
		printf("File Open Error1 !!\n");
		exit(0);
	}

	data = (double**)calloc(TraceNum, sizeof(double*));
	for (i = 0; i < TraceNum; i++) {
		data[i] = (double*)calloc(TraceLength, sizeof(double));
	}

	for (i = 0; i < TraceNum; i++) {
		fread(data[i], sizeof(double), TraceLength, rfp);
	}

	fclose(rfp);

	//plaintext에 평문 저장
	sprintf_s(buf, 256 * sizeof(char), "%s%s", _FOLD_, PlaintextFN);
	if ((err = fopen_s(&rfp, buf, "rb"))) {
		printf("File Open Error2 !!\n");
		exit(0);
	}

	plaintext = (unsigned char**)calloc(TraceNum, sizeof(unsigned char*));
	for (i = 0; i < TraceNum; i++) {
		plaintext[i] = (unsigned char*)calloc(16, sizeof(unsigned char));
	}

	for (i = 0; i < TraceNum; i++) {
		fread(plaintext[i], sizeof(unsigned char), 16, rfp);
	}

	//기존 파형에서 노이즈 제거한 파형 Min 생성
	Sum = (double*)calloc(TraceLength, sizeof(double));
	Mean = (double*)calloc(TraceLength, sizeof(double));
	Min = (double**)calloc(TraceNum, sizeof(double*));
	for (i = 0; i < TraceNum; i++) {
		Min[i] = (double*)calloc(TraceLength, sizeof(double));
	}

	for (i = 0; i < TraceNum; i++) {
		for (j = 0; j < TraceLength; j++) {
			Sum[j] += data[i][j];
		}
	}

	for (i = 0; i < TraceLength; i++) {
		Mean[i] = Sum[i] / TraceNum;	//double을 써야하나? line 418
	}

	for (i = 0; i < TraceNum; i++) {
		for (j = 0; j < TraceLength; j++) {
			Min[i][j] = data[i][j] - Mean[j];
		}
	}

	free(Sum);
	free(Mean);
	free(data);

	//타겟을 서로 곱해 마스킹 지운 파형 Mult 생성
	Mult = (double**)calloc(TraceNum, sizeof(double*));
	
	for (i = 0; i < TraceNum; i++) {
		Mult[i] = (double*)calloc(len, sizeof(double));
	}

	for (i = 0; i < TraceNum; i++) {
		int n = 0;
		for (j = S_Msbox; j < E_Msbox; j++) {
			for (k = S_Msub; k < E_Msub; k++) {
				Mult[i][n] = Min[i][j] * Min[i][k];
				n++;
			}
		}
	}

	free(Min);
	
	//파형 Mult와 Sbox출력값의 상관계수 구해 키 찾기
	Sx = (double*)calloc(len, sizeof(double));
	Sxx = (double*)calloc(len, sizeof(double));
	Sxy = (double*)calloc(len, sizeof(double));
	corrT = (double*)calloc(len, sizeof(double));

	for (i = 0; i < len; i++) {
		for (j = 0; j < TraceNum; j++) {
			Sx[i] += Mult[j][i];
			Sxx[i] += Mult[j][i] * Mult[j][i];
		}
	}

	for (i = 0; i < 16; i++) {
		max = 0;
		maxkey = 0;
		for (key = 0; key < 256; key++) {
			Sy = 0;
			Syy = 0;
			memset(Sxy, 0, sizeof(double) * len);
			for (j = 0; j < TraceNum; j++) {
				iv = Sbox[plaintext[j][i] ^ key];
				hw_iv = 0;

				for (k = 0; k < 8; k++) {
					hw_iv += ((iv >> k) & 1);
				}

				Sy += hw_iv;
				Syy += hw_iv * hw_iv;

				for (k = 0; k < len; k++) {
					Sxy[k] += hw_iv * Mult[j][k];
				}
			}
			for (k = 0; k < len; k++) {
				corrT[k] = ((double)TraceNum * Sxy[k] - Sx[k] * Sy) / sqrt(((double)TraceNum * Sxx[k] - Sx[k] * Sx[k]) * ((double)TraceNum * Syy - Sy * Sy));

				if (fabs(corrT[k]) > max) {
					maxkey = key;
					max = fabs(corrT[k]);
				}
			}
			sprintf_s(buf, 256 * sizeof(char), "%sfinal_corrtrace\\%02dth_block_%02x.corrtrace", _FOLD_, i, key);
			if ((err = fopen_s(&wfp, buf, "wb"))) {
				printf("File Open Error5!!\n");
			}

			fwrite(corrT, sizeof(double), len, wfp);

			fclose(wfp);

			printf(".");
		}
		printf("%02dth_block : maxkey(%02x), maxcorr(%lf)\n", i, maxkey, max);
	}
	free(Sx);
	free(Sxx);
	free(Sxy);
	free(corrT);
	free(plaintext);
}

int main() {
	secondorderCPA();
}
